...
namespace codeavent8_1
{
    ...
    {
        public static IEnumerable<string> Digits()
        {
            return data.Split("\r\n").SelectMany(item => item.Split(" | ")[0].Split(' '));
        }

        public static IEnumerable<string> Outputs()
        {
            return data.Split("\r\n").SelectMany(item => item.Split(" | ")[1].Split(' '));
        }

        public static string data => @"defabc gcb dbafcg gc gcbed fbecgd begfdac fcbde cfge debag | gfce fgce bdefgca aebgd
